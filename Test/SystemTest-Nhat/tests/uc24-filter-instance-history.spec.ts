...
async function findUserFromHistoryRows(page: Page) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const userCell = row.locator("td").nth(4);
        const userTitle = ((await userCell.getAttribute("title")) ?? "").trim();
        const userVisible = ((await userCell.innerText()) ?? "").replace(/\s*#\d+\s*$/g, "").trim();
        const userName = userTitle || userVisible;

        if (userName && userName !== "-" && !/^(none|null)$/i.test(userName)) {
            return userName;
        }
    }
...
test.describe("UC-24 Filter History", () => {
    ...
    test("TC24.06 - Filter theo user từ dữ liệu history hiện có", async ({ page }) => {
        const historyUser = await findUserFromHistoryRows(page);
        const userName = historyUser ?? "admin";
        ...
        // Chỉ assert nội dung khi có user history hợp lệ và sau filter vẫn có data rows.
        if (historyUser) {
            const tbody = page.locator("table.clean-table tbody");
            const bodyText = ((await tbody.textContent()) ?? "").trim();
            if (!/No entries found/i.test(bodyText)) {
                await expect(tbody).toContainText(historyUser);
            }
        }
    });
    ...
    test("TC24.09 - Per_page dropdown có các option 25, 50, 100, 200", async ({ page }) => {
        ...
        const nativePerPageSelect = page.locator('select#per_page, select[name="per_page"]').first();
        const hasNativeSelect = (await nativePerPageSelect.count()) > 0;

        if (hasNativeSelect) {
            const optionValues = await nativePerPageSelect
                .locator("option")
                .evaluateAll((opts) => opts.map((o) => (o as HTMLOptionElement).value.trim()));
            expect(optionValues).toEqual(expect.arrayContaining(expected));
        } else {
            // Fallback cho UI custom-select: verify 4 giá trị boundary được hệ thống chấp nhận.
            for (const val of expected) {
                await page.goto(`${BASE_URL}/admin/instances_history?per_page=${val}`, { waitUntil: "domcontentloaded" });
                await expect(page).toHaveURL(new RegExp(`per_page=${val}`));
                await expect(page.locator("table.clean-table")).toBeVisible();
            }
        }
    });
