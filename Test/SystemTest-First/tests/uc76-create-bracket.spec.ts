...
import { BASE_URL, commitLazyInput, getBrackets, getTeams, loginAsAdmin, openTeamEditModal } from "./support";
...
test.describe("UC-76 Create Bracket", () => {
    ...
    test("CRB-005 - [ECP - Invalid] Tạo bracket với tên chỉ gồm whitespace → Hệ thống cho phép (Success)", async ({ page }) => {
        const description = `UC76_WS_${Date.now()}`;
        await page.goto(`${BASE_URL}/admin/config`, { waitUntil: "domcontentloaded" });
        await page.click('a[href="#brackets"]');
        await page.click('#brackets button:has-text("Add New Bracket")');
        ...
        await commitLazyInput(block.locator("input.form-control").nth(1), description);
        ...
        await expect.poll(async () => {
            ...
            return brackets.some((bracket) => bracket.description === description);
    ...
    test("CRB-006 - Tạo bracket với description trống → Thành công", async ({ page }) => {
        ...
        await expect.poll(async () => {
            const brackets = await getBrackets(page);
            const created = brackets.find((bracket) => bracket.name === name);
            return created ? created.description : undefined;
        }, { timeout: 15_000 }).toBe("");
    });
    ...
    test("CRB-007 - Tạo bracket với description rất dài (5.000 ký tự)", async ({ page }) => {
        ...
        await expect.poll(async () => {
            const brackets = await getBrackets(page);
            const created = brackets.find((bracket) => bracket.name === name);
            return created?.description.length ?? 0;
        }, { timeout: 15_000 }).toBe(5000);
    });
