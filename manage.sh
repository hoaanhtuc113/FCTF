...
EOF
    ...
    # Cập nhật appsettings.json cho MQ_Producer
    cat > "$PROJECT_ROOT/ControlCenterAndChallengeHostingServer/MQ_Producer/bin/Release/net8.0/linux-x64/publish/appsettings.json" << EOF
{
    "Logging": {
    "LogLevel": {
      "Default": "Information",
...
build_apps() {
    ...
    # Kiểm tra file dự án MQ_Producer
    if [ ! -f "MQ_Producer/MQ_Producer.csproj" ]; then
        echo "Lỗi: File dự án MQ_Producer/MQ_Producer.csproj không tồn tại."
        exit 1
    fi
    if ! dotnet publish MQ_Producer/MQ_Producer.csproj -c Release --framework net8.0 --runtime linux-x64 --self-contained true; then
    ...
    if [ ! -f "ControlCenterServer/bin/Release/net8.0/linux-x64/publish/ControlCenterServer" ]; then
        ...
        if [ ! -d "MQ_Producer/bin/Release/net8.0/linux-x64/publish" ]; then
        echo "Lỗi: Không thể tạo thư mục publish cho MQ_Producer."
        exit 1
    fi
    if [ ! -f "MQ_Producer/bin/Release/net8.0/linux-x64/publish/MQ_Producer" ]; then
