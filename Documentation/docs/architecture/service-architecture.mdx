...
:::tip[Runtime Boundary Highlight]
...
:::important[Control-plane contract]
...
- Dead letter exchange deployment_dlx (direct) routes rejected and expired messages
  to deployment_dlq, which holds them for 7 days or 1000 messages, dropping the
  oldest first. Overflow rejections are not dead-lettered: they are refused at
  publish time and surface to the caller as a 429 instead.
