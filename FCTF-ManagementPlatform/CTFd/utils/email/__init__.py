...
def sendmail(addr, text, subject="Message from {ctf_name}", is_html=False):
    ...
    return EmailProvider.sendmail(addr, text, subject, is_html)
...
def user_created_notification(addr, name, password, is_html = True):
    text = safe_format(
    ...
    return sendmail(addr=addr, text=text, subject=subject, is_html=is_html)
