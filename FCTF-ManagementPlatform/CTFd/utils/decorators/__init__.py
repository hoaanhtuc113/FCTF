...
def challenge_writer_only(f):
    @functools.wraps(f)
    def challenge_writer_only_wrapper(*args, **kwargs):
    ...
    return challenge_writer_only_wrapper


def jury_only(f):
    @functools.wraps(f)
    def jury_only_wrapper(*args, **kwargs):
        if is_jury():
            return f(*args, **kwargs)
        else:
            if request.content_type == "application/json":
...
def admin_or_challenge_writer_only(f):
    ...
    def admin_or_challenge_writer_only_wrapper(*args, **kwargs):
        ...
        if is_challenge_writer() or is_admin():
            return f(*args, **kwargs)
        else:
            if request.content_type == "application/json":
                abort(403)
...
def admin_or_jury(f):
    ...
    return admin_or_jury_only_wrapper
def require_team(f):
    @functools.wraps(f)
